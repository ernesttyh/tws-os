  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex gap-1 bg-gray-50 rounded-lg p-1 w-fit">
        ...
        <button onClick={() => setSubTab('shoots')} className={`px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition ${subTab === 'shoots' ? 'bg-purple-600 text-white' : 'text-gray-500 hover:text-gray-900'}`}>
        ...
        <div className="space-y-3 sm:space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2 sm:gap-3">
              <select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className="bg-gray-50 border border-gray-200 rounded-lg px-2 sm:px-3 py-2 text-gray-900 text-xs sm:text-sm">
              ...
              <div className="flex gap-1 bg-gray-50 rounded-lg p-0.5">
                <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-gray-100 text-gray-900' : 'text-gray-500'}`}><Grid3x3 size={14} /></button>
                <button onClick={() => setViewMode('list')} className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-gray-100 text-gray-900' : 'text-gray-500'}`}><List size={14} /></button>
              </div>
              <span className="text-xs sm:text-sm text-gray-500">{content.length} items</span>
            </div>
          ...
          {content.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {content.map(c => (
                <div key={c.id} className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200 hover:border-gray-300 transition group">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => { setEditContent(c); setContentForm({ title: c.title || '', contents: c.contents || '', content_type: c.content_type, status: c.status, date: c.date || '', day_of_week: c.day_of_week || '', caption: c.caption || '', hashtags: c.hashtags || '', link: c.link || '', schedule_time: c.schedule_time || '', comments: c.comments || '' }); setShowContentModal(true); }} className="p-1 hover:bg-gray-100 rounded text-gray-500"><Edit2 size={12} /></button>
                      <button onClick={() => deleteContent(c.id)} className="p-1 hover:bg-gray-100 rounded text-gray-500 hover:text-red-400"><Trash2 size={12} /></button>
                    </div>
                  </div>
                  <h3 className="text-xs sm:text-sm font-medium text-gray-900 mb-1">{c.title || c.contents?.slice(0, 50) || 'Untitled'}</h3>
                  {c.contents && <p className="text-[10px] sm:text-xs text-gray-500 line-clamp-2 mb-2">{c.contents}</p>}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-gray-200">
                    <th className="text-left py-3 px-3 sm:px-4 text-xs font-medium text-gray-500">Date</th>
                    {selectedMonth === 'All' && <th className="text-left py-3 px-3 sm:px-4 text-xs font-medium text-gray-500">Month</th>}
                    <th className="text-left py-3 px-3 sm:px-4 text-xs font-medium text-gray-500">Type</th>
                    <th className="text-left py-3 px-3 sm:px-4 text-xs font-medium text-gray-500">Content</th>
                    <th className="text-left py-3 px-3 sm:px-4 text-xs font-medium text-gray-500 hidden sm:table-cell">Status</th>
                    <th className="text-right py-3 px-3 sm:px-4 text-xs font-medium text-gray-500">Actions</th>
                  </tr></thead>
                  <tbody>
                    {content.map(c => (
                      <tr key={c.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                        <td className="py-3 px-3 sm:px-4 text-gray-600 text-xs">{c.date ? new Date(c.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : '-'}</td>
                        {selectedMonth === 'All' && <td className="py-3 px-3 sm:px-4 text-gray-500 text-xs">{c.month}</td>}
                        <td className="py-3 px-3 sm:px-4"><StatusBadge status={c.content_type} /></td>
                        <td className="py-3 px-3 sm:px-4 text-gray-600 max-w-[150px] sm:max-w-[300px] truncate text-xs sm:text-sm">{c.title || c.contents?.slice(0, 60) || '-'}</td>
                        <td className="py-3 px-3 sm:px-4 hidden sm:table-cell"><StatusBadge status={c.status} /></td>
                        <td className="py-3 px-3 sm:px-4 text-right">
                          <button onClick={() => { setEditContent(c); setContentForm({ title: c.title || '', contents: c.contents || '', content_type: c.content_type, status: c.status, date: c.date || '', day_of_week: c.day_of_week || '', caption: c.caption || '', hashtags: c.hashtags || '', link: c.link || '', schedule_time: c.schedule_time || '', comments: c.comments || '' }); setShowContentModal(true); }} className="p-1 hover:bg-gray-100 rounded text-gray-500"><Edit2 size={14} /></button>
                          <button onClick={() => deleteContent(c.id)} className="p-1 hover:bg-gray-100 rounded text-gray-500 hover:text-red-400 ml-1"><Trash2 size={14} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          {shoots.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
              {shoots.map(s => (
                <div key={s.id} className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200 hover:border-gray-300 transition">
                  <div className="flex items-start justify-between mb-2 sm:mb-3">
                    <div className="min-w-0">
                      <h3 className="text-gray-900 font-medium text-sm">{s.title}</h3>
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <StatusBadge status={s.status} />
                        {s.shoot_date && <span className="text-[10px] sm:text-xs text-gray-500">{new Date(s.shoot_date).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}</span>}
                        {s.shoot_time && <span className="text-[10px] sm:text-xs text-gray-500">@ {s.shoot_time}</span>}
                      </div>
                    </div>
                    <button onClick={() => { setEditShoot(s); setShootForm({ title: s.title, shoot_date: s.shoot_date || '', shoot_time: s.shoot_time || '', location: s.location || '', status: s.status, shot_list: s.shot_list || '', talent: s.talent || '', client_requirements: s.client_requirements || '', notes: s.notes || '' }); setShowShootModal(true); }} className="p-1.5 hover:bg-gray-100 rounded text-gray-500 shrink-0"><Edit2 size={14} /></button>
                  </div>
                  {s.location && <div className="text-[10px] sm:text-xs text-gray-500 mb-1 sm:mb-2">📍 {s.location}</div>}
                  {s.talent && <div className="text-[10px] sm:text-xs text-gray-500 mb-1 sm:mb-2">🎭 Talent: {s.talent}</div>}
                  {s.shot_list && <div className="text-[10px] sm:text-xs text-gray-600 bg-black/20 rounded p-2 whitespace-pre-wrap line-clamp-4">{s.shot_list}</div>}
                </div>
              ))}
            </div>
          )}
